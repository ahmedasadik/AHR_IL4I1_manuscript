# This is script is for the analysis of the microarray of U87 IL4I1 overexpressing cells
...
setwd("./IL4I1/Results_U87_ovr_xprsn/")
dir.create("./RDS")
...
raw_path <- "/home/data/Raws/IL4I1/ctrl_ovrxprsn/"
list_cels <- list.files(raw_path, pattern = "CEL$", full.names = T)
list_cels <- list_cels[-c(4,8)]
raws_cel <- read.celfiles(list_cels)
saveRDS(raws_cel, "./RDS/raws_cel_u87.rds")
...
info <- data.frame(sample_id=sample_id, condition=condition)
...
write.table(tt_2sd_IL4I1_u, "./TopTables/IL4I1_tt_2sd_before_annot_u87.txt", row.names = F, quote = F, sep = "\t")
...
write.table(tt_2sd_IL4I1_u, "./TopTables/IL4I1_tt_2sd_after_annot_u87.txt", row.names = F, quote = F, sep = "\t")

## Volcano_plot for AHR targets
vp_tt <- tt_2sd_IL4I1_u[!is.na(tt_2sd_IL4I1_u$AHR_target),]

pdf("./Figures/volcano_plot_top10_AHR_targets_U87.pdf", width = 12, height = 8, pointsize = 24)
ggplot(vp_tt, aes(x=logFC, y=(-log10(P.Value)),label=as.character(vp_tt$hGene)))+geom_point(size=5, color="red")+theme_bw()+
  ...
  geom_text(aes(label=ifelse(vp_tt$logFC >= 1,vp_tt$hGene,'')),hjust=1.25, vjust=0.6, color="black")+
  geom_text(aes(label=ifelse(vp_tt$logFC < 1 & vp_tt$logFC >= 0.87,vp_tt$hGene,'')),hjust=-0.35, vjust=0.6, color="black")
dev.off()
...
write.table(TF_reps_comb, "./TFSDB/comb_comp_numbers_comparison_TFs_AHR_u87.txt", sep = "\t", row.names = F)
...
write.table(TF_reps_motifs, "./TFSDB/motifs_comp_numbers_comparison_TFs_AHR_u87.txt", sep = "\t", row.names = F)
stopCluster(cl)
...
walk(1:length(gsa_res), gsa_bar_plot_FUN, gsa_ls=gsa_res, res_path="/Figures/",coi="IL4I1_ctrl_ovrxprsn_u87", wd=8, ht=6, pix=300)
...
walk2(list(TFSBDB_list_comb, TFSBDB_list), c(FALSE, TRUE), function (a,b,c,d,e,f,g){
  ...
  c = tt_2sd_IL4I1_u, d = rma_2sd, e = 2, f = design_eset, g="./TFSDB/u87_")
...
write.table(AHR_hi_lo_roast, "./GSA/AHR_enrichment_U87.txt", sep = "\t")

pdf("./Figures/barcodeplot_m_t_stat_U87.pdf", width = 8, height = 6)
...
saveRDS(cor_matrix_sp, "./cor_matrix_sp_ctrl_ovrxprsn_u87.rds")
saveRDS(cor_matrix_pr, "./cor_matrix_pr_ctrl_ovrxprsn_u87.rds")
## DONE
