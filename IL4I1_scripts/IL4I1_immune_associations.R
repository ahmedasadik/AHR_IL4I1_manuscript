...
# Infiltration scores
TCGA_IS2 <- readRDS("../Zenodo_download/TCGA_GSVA_voom_28_cell_types_IPS.rds")
...
TCGA_modules <- readRDS("../Zenodo_download/TCGA_TOMS_bicor.rds")

# Load TCGA modules genes
...
pdf("../Results/Figures/Inf_cells_heatmap_logFC_0.3_apv_0.05_IL4I1_IFNG.pdf")
...
TCGA_combined <- map(1:32, function(i){
  c_l <- list(wgcna=t(TCGA_modules[[i]]$MEs), immune=TCGA_IS2[[i]])
...
saveRDS(TCGA_MOFA,"../Results/RDS/TCGA_MOFA_wgcna_immune.RDS")
...
LFs_to_show <- map2(TCGA_MOFA,IL4I1_modules,function(mof_obj,mod_name){
  wts <- map(1:5,function(i,w_v){
    df <- w_v@Expectations$W$wgcna[,i]%>% as.data.frame()
    df$. <- df$./max(abs(df$.))
    df$new <- abs(df$.)
    df <- df[order(df$new,decreasing = T),]
    gsub("^ME","",rownames(df)[1:10])
  },w_v=mof_obj)
  names(wts) <- paste("LF",1:5,sep = "")
  idcs_list <- map(wts,function(x){
    which(x==mod_name)
  })
  res <- map(idcs_list, function(id){
    map(id,length)%>%unlist()
  })%>% unlist()
}) %>% map(.,function(z){ gsub("LF","",(names(z))[1])}) %>% unlist()
...
wgcna_weights <- map2(to_subset,as.numeric(LFs_to_show),function(mof_obj,factor,view){
...
immune_weights <- map2(to_subset,as.numeric(LFs_to_show),function(mof_obj,factor,view){
  W <- getWeights(TCGA_MOFA[[mof_obj]], views = view, factors = factor, as.data.frame = TRUE)
  W$value <- W$value/max(abs(W$value))
  W$group <- "0"
  W$group[abs(W$value) >= sort(abs(W$value), decreasing = TRUE)[10]] <- "1"
...
pdf("../Results/Figures/MOFA_circos.pdf",width = 8,height = 8)
circos_figure()
